...
searchBtn.style = "width: 30px; border : solid grey; border-radius : 5px;margin-left: 45%;margin-right: 45%;margin-top: 7px;"
          ...
          <div class="row">  
            ...
            <div id="radio"class="col-8 pt-2" style="display : flex;">
              ...
              <label class="filter col-2 container"> Nom de la personne
                <input id="name" type="radio" checked="checked" name="radio">
  ...
  searchButton.onclick = async () => 
    {
      ...
      const filter = document.getElementById("type").checked ? "type" 
      : document.getElementById("state").checked ? "state" 
      : document.getElementById("name").checked ? "name" : "";
      
      console.log(value + filter);
      try {
        const response = await fetch("/api/items/filtered?filter=" + filter + "&input="+ value); // fetch return a promise => we wait for the response
        console.log("res", response.body);
        if (!response.ok) {
          throw new Error(
              "fetch error : " + response.status + " : " + response.statusText
    ...
    type.onclick = () => {radio.innerHTML = input2;}
    name.onclick = () => {radio.innerHTML = input2;}
      ...
      searchBtn.onclick = async () => 
        {
          ...
          try {
          const response = await fetch("api/items/filtered?filter=date&input=" + date1 + "-" + date2, options);

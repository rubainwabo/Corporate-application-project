...
const MemberList = async () => {
  ...
  document
    .getElementById("container-i-member-list")
  ...
  try {
    ...
    filtredUserList.forEach((e) => {
      ...
      divUserHandler.addEventListener("click", async () => {
        ...
        let offeredItems = await getAllItemsByItemCondition(
          "offered",
          token,
          divUserHandler.id,
          true
        );
        let receptedItems = await getAllItemsByItemCondition(
          "gifted",
          token,
          divUserHandler.id,
          false
...
function autocomplete(inp, token) {
  ...
  inp.addEventListener("input", async function (e) {
    ...
    try {
      ...
      const response = await fetch(
        "/api/admins/autocompleteList?value=" + val.toUpperCase(),
        options
      );
      if (response.status == 307) {
        await VerifyUser();
